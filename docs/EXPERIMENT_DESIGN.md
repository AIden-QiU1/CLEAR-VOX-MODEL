...
## 📊 实验结果更新 (2025-12-25) - 里程碑

### EXP-002: 1h微调 ✅ 完成

| 指标 | 值 | 说明 |
|------|-----|------|
| **模型** | Paraformer-large + 1h微调 | batch_size=24, SpecAugment |
| **训练数据** | 45,327 样本 (35人) | |
| **验证数据** | 4,460 样本 (4人) | |
| **测试数据** | 6,064 样本 (5人) | 完整测试集 |
| **训练轮次** | **Epoch 28/50** | OOM @ epoch 28 |
| **最佳模型** | **Epoch 26** | model.pt.best |
| **训练总步数** | 53,648 steps | |
| **训练时长** | ~4小时 | |
| **状态** | ✅ 已完成 | 2025-12-25 |
...
| Cache 峰值 | 23.242 GB |
| 设备 | 24GB GPU |
| 中断原因 | CUDA OOM @ epoch 28 |

---

### 📈 完整测试集评估结果

| 指标 | 基线 (EXP-001) | 微调后 (EXP-002) | 改进 |
|------|----------------|------------------|------|
| **模型** | Paraformer-large (原始) | Paraformer-large + 1h微调 | - |
| **测试样本** | 100 (部分) | **6,064 (完整)** | ✓ |
| **CER** | 88.73% | **66.85%** | **↓ 21.88%** |
| **错误样本** | N/A | 0 | ✓ |
| **推理速度** | - | ~6 it/s | 16分50秒 |

#### 关键发现

1. ✅ **CER 从 88.73% 降至 66.85%** - 显著改进 21.88 个百分点
2. ✅ 模型开始学习构音障碍语音的特征模式
3. ⚠️ 仍距离目标 CER < 20% 有较大差距
4. ⚠️ batch_size=24 导致显存占用过高，在 epoch 28 触发 OOM
...
```
/root/autodl-tmp/exp/paraformer_finetune_1h_optimized/
├── model.pt.best              # 最佳模型 (Epoch 26)
├── model.pt                   # 最新模型
├── model.pt.ep16-ep27         # 各 epoch 保存点
├── test_results_complete.json # 完整测试结果 (6,064样本)
...
| 任务 | 优先级 | 说明 |
|------|--------|------|
| 继续训练到 50 epoch | P0 | 使用 batch_size=16 避免 OOM |
| 优化显存使用 | P1 | 梯度检查点 + 混合精度 |
| 准备 10h 数据集 | P1 | 开始 EXP-003 |
| 分析错误样本 | P2 | 了解模型薄弱环节 |
...
| 实验ID | 实验名称 | 状态 | CER | 说明 |
|--------|----------|------|-----|------|
| EXP-001 | Baseline | ✅ 完成 | 88.73%* | *100样本测试 |
| EXP-002 | 1h微调 | ✅ 完成 | **66.85%** | 6,064样本完整测试 |
| EXP-003 | 10h微调 | ⬜ 待执行 | - | - |
| EXP-004 | 11h联合 | ⬜ 待执行 | - | - |
| EXP-005 | 跨说话人 | ⬜ 待执行 | - | - |
| EXP-006 | 少样本适应 | ⬜ 待执行 | - | - |
